...
func CreateALIClusterOnAlibaba(csClient *cs.Client, region string, clusterName string, k8sVersion string, nodes string, tags map[string]string, extraArgs ...string) (string, error) {
	...
	nodepool := &cs.Nodepool{
		...
		NodepoolInfo: &cs.NodepoolNodepoolInfo{
			...
			ResourceGroupId: tea.String(aliClusterConfig.ResourceGroupID),
		},
...
// ListALIAllVersions lists all available ALI Kubernetes versions
...
func ImportALIHostedCluster(client *rancher.Client, clusterName, cloudCredentialID, region string, clusterId string) (*management.Cluster, error) {
...
func DeleteALIClusteronAlibaba(csClient *cs.Client, clusterId string) error {
...
func UpgradeClusterKubernetesVersion(cluster *management.Cluster, upgradeToVersion string, client *rancher.Client, checkClusterConfig bool) (*management.Cluster, error) {
	...
	//currentVersion := *&cluster.AliConfig.KubernetesVersion
	...
	if checkClusterConfig {
		...
		Eventually(func() string {
			ginkgo.GinkgoLogr.Info("Waiting for k8s upgrade to appear in ALIStatus.UpstreamSpec ...")
			cluster, err = client.Management.Cluster.ByID(cluster.ID)
			Expect(err).To(BeNil())
			return cluster.AliStatus.UpstreamSpec.KubernetesVersion
