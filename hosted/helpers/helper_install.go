...
func InstallAlibabaOperatorCharts(k *kubectl.Kubectl, chartVersion, chartRegistry string) {
	...
	if chartVersion == "" {
		By("Fetching appropriate Alibaba operator chart version for Rancher", func() {
			...
			Expect(chartVersion).ToNot(BeEmpty(), "Failed to auto-detect Alibaba chart version for Rancher %s; set ALIBABA_OPERATOR_VERSION explicitly", RancherFullVersion)
